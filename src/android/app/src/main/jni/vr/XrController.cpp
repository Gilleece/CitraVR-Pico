...
void SuggestBindingsForProfile(const XrInstance& instance, const char* interactionProfileName,
                               const std::vector<XrActionSuggestedBinding>& bindings) {
    XrPath interactionProfilePath = XR_NULL_PATH;
    const XrResult profilePathResult =
        xrStringToPath(instance, interactionProfileName, &interactionProfilePath);
    if (XR_FAILED(profilePathResult)) {
        OXR_CheckErrors(profilePathResult, "xrStringToPath(interaction profile)", false);
        return;
    }
    ...
    OXR_CheckErrors(xrSuggestInteractionProfileBindings(instance, &suggestedBindings),
                    "xrSuggestInteractionProfileBindings", false);
}
...
InputStateStatic::InputStateStatic(const XrInstance& instance, const XrSession& session) {
    ...
    // Create bindings for XR controllers.
    {
        // Map bindings
        ...
        SuggestBindingsForProfile(instance, "/interaction_profiles/oculus/touch_controller",
                                  bindings);
        SuggestBindingsForProfile(instance, "/interaction_profiles/bytedance/pico4_controller",
                                  bindings);
        SuggestBindingsForProfile(instance, "/interaction_profiles/bytedance/pico_neo3_controller",
                                  bindings);
